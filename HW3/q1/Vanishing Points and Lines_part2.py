...
# ----------------------------------------------
res03 = np.copy(image)
cv2.circle(res03, (int(px), int(py)), radius=20,
           color=(255, 0, 0), thickness=20)
plt.imshow(res03)
plt.title(f"focal length : {int(focal_len)}px")
plt.savefig("res03.jpg")
plt.show()

print(f"principal point : {(px,py)}")
print(f"focal length : {focal_len}")
print(f"calibratin matrix : {K}")
print(f"z_rotation : {np.round(z_rotation*180/np.pi,2)} deg")
print(f"x_rotation : {np.round(x_rotation*180/np.pi,2)} deg")
